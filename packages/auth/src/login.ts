...
import { readRateLimitInts, tryConsumeSimpleRateLimit, } from "./security/rate-limit";
import { jitterAuthFailure } from "./security/timing";
...
export const loginUser = async (raw: unknown, ipKey: string): Promise<LoginServiceResult> => {
    const { max: loginMax, windowMs: loginWindow } = readRateLimitInts(process.env.LOGIN_RATE_LIMIT_MAX, process.env.LOGIN_RATE_LIMIT_WINDOW_MS, { max: 40, windowMs: 15 * 60 * 1000 });
