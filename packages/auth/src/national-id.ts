...
const expectedChecksumChar = (first17Upper: string): string => {
    let sum = 0;
    for (let i = 0; i < 17; i++) {
...
const isProbableAdministrativeCodes = (code: string): boolean => {
    return /^[1-9]\d{5}$/.test(code) && code !== "000000";
};
const isReasonableBirthYmd = (y: number, m: number, d: number): boolean => {
    const now = new Date();
    const thisYear = now.getFullYear();
    if (!(y >= 1850 && y <= thisYear))
        return false;
    if (!(m >= 1 && m <= 12))
        return false;
    if (!(d >= 1 && d <= 31))
...
export const isValidChineseResidentId = (idNormalizedUpper: string): boolean => {
    if (!/^[1-9]\d{16}[\dX]$/.test(idNormalizedUpper))
        return false;
    const first17 = idNormalizedUpper.slice(0, 17);
    const checksum = idNormalizedUpper.slice(17);
    if (expectedChecksumChar(first17) !== checksum)
        return false;
    const admin = idNormalizedUpper.slice(0, 6);
    if (!isProbableAdministrativeCodes(admin))
        return false;
