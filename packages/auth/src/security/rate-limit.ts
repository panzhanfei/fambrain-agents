...
function bucketStore(): Map<string, RlBucket> {
  const g = globalThis as typeof globalThis & { __fambrainRateLimit?: Map<string, RlBucket> };
...
function pruneStale(nowMs: number) {
  if (nowMs - lastPruned < 60_000) return;
  lastPruned = nowMs;
  const m = bucketStore();
  for (const [k, v] of m) {
    if (v.resetAt <= nowMs) m.delete(k);
  }
}

/**
 * 固定窗口 + 计数器内存限流（单机部署适用；多副本需网关或 Redis）。
 */
export function tryConsumeSimpleRateLimit(
  key: string,
  limit: number,
  windowMs: number,
...
export function readRateLimitInts(
