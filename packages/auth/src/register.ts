...
export const registerUser = async (raw: unknown, ipKey: string): Promise<RegisterServiceResult> => {
    const { max: regMax, windowMs: regWindow } = readRateLimitInts(process.env.REGISTER_RATE_LIMIT_MAX, process.env.REGISTER_RATE_LIMIT_WINDOW_MS, { max: 12, windowMs: 60 * 60 * 1000 });
    ...
    const principal = !isBootstrapFirstUser
