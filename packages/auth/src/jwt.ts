...
const jwtEpochSeconds = (v: unknown): number | undefined => {
...
export const verifyJwt = async (raw: string): Promise<VerifiedJwt> => {
    const sk = getJwtSecretKey();
...
export const verifyAuthToken = async (raw: string): Promise<string> => {
