 ...
 * 离线 CLI：扫描 src/doc/users/corpus → 分块 → Ollama embed → Chroma。
 */
...
export type { ChunkMetadata } from "@/lib/knowledge/chunk-metadata";
export { chunkMetadataSchema } from "@/lib/knowledge/chunk-metadata";
...
export async function indexAllCorpora(): Promise<void> {
  ...
  if (userIds.length === 0) {
    indexerLogger.warn("no users with corpus markdown under src/doc/users/");
    return;
  }
