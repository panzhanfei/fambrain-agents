...
import { matchUiEnumerationPrompt } from "../enumeration-action-prompts";
...
export const runIntakeNode = async (
    ...
    try {
        ...
        /**
         * 步骤 0：UI 按钮精确匹配（ENUMERATION_ACTION_PROMPTS）— 不调 LLM。
         * 仅认 Analyst 发出的固定 prompt，无口语 regex 词表。
         */
        const uiControl = matchUiEnumerationPrompt(state.userQuestion);
        if (uiControl && (uiControl.action === "continue" || uiControl.action === "exhaustive")) {
            const stored = await getEnumerationListSession(
                session,
                uiControl.listKind
            );
            const page =
                uiControl.action === "continue"
                    ? (stored?.lastPage ?? 1) + 1
                    : 1;
            const pageSize =
                stored?.pageSize ?? ENUMERATION_EXHAUSTIVE_PAGE_SIZE;
            return {
