    ...
    for (const item of items) {
        ...
        if (item.queryType === "enumeration" || prev.queryType === "enumeration") {
            ...
            byKey.set(key, {
                ...
                topics:
                    prev.topics.length > 0 ? prev.topics : [...item.topics],
                enumerationControl: {
                    action,
                    listKind,
                    excludeHint,
...
export const normalizePlanItemFromSchema = (
    ...
    if (queryType === "enumeration") {
        const listKind =
            item.enumerationControl?.listKind ??
            (item.topics.includes("project") ||
            item.topics.includes("tech-stack")
                ? "project"
                : item.topics.includes("experience") ||
                    item.topics.includes("career")
                  ? "experience"
                  : null);
        if (!listKind) {
            return {
        ...
        return {
            ...
            enumerationControl: {
                ...
                timeWindowYears:
                    tw != null && Number.isFinite(tw) && tw > 0
                        ? Math.floor(tw)
                        : null,
            },
        };
...
/**
 * @deprecated 口语拆槽已移除；保留导出供旧测试迁移，行为 = 单条 normalize。
 */
export const annotatePlanItem = (
    item: IntakeRetrievalPlanItem
): IntakeRetrievalPlanItem[] => [normalizePlanItemFromSchema(item)];

/**
 * 修复多问 plan：
 * - 空/过少 plan 时用 subTasks 建 default 项（结构切分，不猜类型）
 * - schema 合法化 + 结构化去重
 */
export const repairRetrievalPlanItems = (
    ...
    let items = plan.filter(
        (p) => p.label.trim() && p.searchQuery.trim()
    );

    // 仅当 LLM 未给出任何 plan：用 subTasks 建 default 壳（不猜 queryType）
    if (items.length === 0 && subTasks.length >= 2) {
        items = subTasks.map((label) => ({
            label,
            searchQuery: label,
