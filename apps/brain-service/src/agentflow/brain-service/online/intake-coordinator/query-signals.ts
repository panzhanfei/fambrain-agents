...
import { looksLikeMultiPartQuestion } from "@/agentflow/brain-service/online/intake-coordinator/composite";
...
export const lastSubstantiveUserQuestion = (
    ...
    for (let i = history.length - 1; i >= 0; i -= 1) {
        ...
        if (q.length >= 4 && !isShortContinuationUtterance(q)) return q;
    }
    return null;
};
