...
import { runIntakePipeline } from "../src/agentflow/brain-service/online/intake-coordinator/pipeline/intake-pipeline";
...
assertSync("Intake LLM 声明 external_link → 保留路由", () => {
...
assertSync("「不止这一个」+ 上文 GitHub → retrieve 非 clarify", () => {
...
assertSync("泛化「开源两个项目 GitHub」→ 单槽简历对外链接（不继承旧 subTasks）", () => {
...
assertSync("编号双问 GitHub → 2 槽 external_link（实体级 searchQuery）", () => {
    const userQuestion =
