...
import { docParserLogger, ingestDocumentBatch, isSupportedDocFile, } from "@fambrain/agents";
type CliOptions = {
    actorUserId: string;
    corpusUserId: string;
...
const parseArgs = (argv: string[]): CliOptions => {
    ...
    if (!actorUserId || paths.length === 0) {
        console.error("用法: pnpm run parse:documents -- <actorUserId> <fileOrDir...> [--category personal|projects|experience] [--corpus-user <id>] [--no-index]");
        process.exit(1);
    }
    return {
        actorUserId,
...
const main = async () => {
    ...
    const files = await Promise.all(filePaths.map(async (absPath) => ({
