...
export const indexAllCorpora = async (): Promise<void> => {
    ...
    for (const corpusUserId of userIds) {
        ...
        indexerLogger.info({ corpusUserId, fileCount, chunkCount, durationMs: Date.now() - t0 }, "user done");
    }
    indexerLogger.info({ userCount: userIds.length, totalFiles, totalChunks }, "index all completed");
};
