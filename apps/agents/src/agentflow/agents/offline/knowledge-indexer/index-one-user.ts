...
export const indexOneCorpusUser = async (corpusUserId: string, logger: Logger): Promise<IndexOneUserResult> => {
    ...
    for (const absPath of mdFiles) {
        ...
        docs.push(...splitMarkdownToDocuments(corpusUserId, repoPath, body, fileName));
    }
    if (docs.length === 0) {
    ...
    const { collectionName, chunkCount } = await indexCorpusDocuments(corpusUserId, docs, logger, embedOptions);
    logger.info({
        corpusUserId,
        collectionName,
        fileCount: mdFiles.length,
