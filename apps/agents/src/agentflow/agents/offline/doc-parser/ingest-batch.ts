...
export function getDocParseConcurrency(): number {
...
async function ingestOneFile(
  ...
  try {
    const { vaultRelativePath, corpusRelativePath, mdFileName } =
      buildOutputPaths(actorUserId, corpusUserId, category, fileName);
    ...
    const writtenPath = await writeParsedToCorpus(
      corpusUserId,
      category,
      mdFileName,
      parsed
...
/** 批量上传 → 解析 → 写 corpus Markdown；可选触发 Chroma 全量入库。 */
export async function ingestDocumentBatch(
  files: UploadFileInput[],
  options: IngestBatchOptions
): Promise<DocParseBatchResult> {
  const { actorUserId, corpusUserId, category, indexAfter = true, logger } =
    options;
  ...
  const results = await Promise.all(
    files.map((file) => limit(() => ingestOneFile(file, options)))
  );

  const successCount = results.filter((r) => r.ok).length;
  logger?.info(
    { successCount, total: results.length },
    "doc parse batch files done"
  );

  let indexResult: { fileCount: number; chunkCount: number } | undefined;
  let indexed = false;
