...
import { buildRuleBasedFactCheck, normalizeFactCheckerResult, parseJsonObject, } from "./check-helpers";
...
export const completeFactCheck = async (input: FactCheckerInput): Promise<FactCheckerResult> => {
    ...
    try {
        ...
        const result = normalizeFactCheckerResult(parsed, fallback, input.retryCount);
        logAgentStep("FactChecker", "步骤6 · Zod 规范化后", {
            ...
            changedFromLlm: beforeNormalize !== null &&
        ...
        logAgentStep("FactChecker", "步骤8 · 最终判定", {
            ...
            willRetryRetrieval: !result.passed && result.refinedSearchQuery !== null && input.retryCount < 1,
