...
function mergeExcerptText(a: string, b: string): string {
  const x = a.trim();
  const y = b.trim();
  if (!x) return y;
  if (!y) return x;
  ...
  const merged = `${x} / ${y}`;
  if (merged.length <= EXCERPT_MAX) return merged;
...
/**
 * 按 path 合并 hits：保留最高 relevance，excerpt 去重后拼接。
 */
export function organizeHits(hits: KnowledgeHit[]): KnowledgeHit[] {
...
/** 分析师终稿 citations：同 path 合并 excerpt */
