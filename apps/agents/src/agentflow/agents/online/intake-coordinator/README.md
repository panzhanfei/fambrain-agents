...
Intake 是 Pipeline 的**第一个在线 Agent**。它把用户自然语言变成一张**结构化路由工单**，告诉下游「要不要查库、查什么、单问还是多问分槽、还是直接短答 / 记 QQ」。

**Intake 不做的事：** 检索语料、事实核查、写最终长分析、读写数据库。
...
| 同句再问浪费算力 | **L1 repeat cache**，复用 history 里的 assistant 答 |
...
| Mem0 / LangMem | 由 `stream.ts` 注入 `memoryBlock` | 帮理解多轮指代，**不能**替代 searchQuery 里的实体词 |
...
Web 用户消息
    │
    ▼
pipeline/graph/stream.ts
    ├─ [L1] findRepeatAnswerInHistory()     guards/intake-repeat-guard.ts
    │         命中 → 直接返回答案，跳过 Intake LLM
    │
    ├─ preparePipelineMemory()              packages/agent-memory
    │         → memoryBlock, intakeHistory, userMemories
    │
    ▼
pipeline/graph/compile.ts  →  intakeNode()
    │
    ├─ findRepeatAnswerInHistory()          （图内再检一次 L1）
    │
...
history 中已有:
  user: "城管平台用了什么技术"
  assistant: "（长答…）"
  user: "城管平台用了什么技术"   ← 同句再问

findRepeatAnswerInHistory → 直接返回答案
→ 跳过 LLM / KM / FC / Analyst
...
| 文件 | 职责 |
|------|------|
| `intake-repeat-guard.ts` | L1：同 normalize 问法复用 history assistant 答 |
...
| 调用方 | 使用的 Intake 导出 |
|--------|-------------------|
| `pipeline/graph/compile.ts` | `completeIntakeCoordinator`, `runIntakePipeline`, `findRepeatAnswerInHistory`, `resolveIncrementalCompositePlan` |
| `pipeline/graph/stream.ts` | `findRepeatAnswerInHistory`（L1，图外） |
...
可选标记：`state.repeatQuestionHit`（L1，在 intakeNode 或 stream 层设置）。
