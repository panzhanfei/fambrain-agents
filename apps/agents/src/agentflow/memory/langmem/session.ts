...
function formatTurns(turns: DbChatTurn[]): string {
  return turns
...
/** 历史过长时只保留最近 N 轮给 Intake（摘要另注入） */
export function trimHistoryForIntake(history: DbChatTurn[]): DbChatTurn[] {
  const cfg = getMemoryConfig();
...
export async function summarizeSessionTurns(
...
export async function persistSessionSummary(
  ...
  try {
    ...
    await writeFile(
      sessionFilePath(conversationId),
