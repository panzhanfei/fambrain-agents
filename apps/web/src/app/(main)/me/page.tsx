...
const MePage = async () => {
    ...
    if (!session)
        return null;
    return (<div className="mx-auto flex min-h-dvh max-w-lg flex-col gap-6 px-6 py-10 text-[15px] text-[#374151]">
      <header>
      ...
      <nav className="flex flex-wrap gap-3">
        <Link href="/" className="rounded-full border border-[#e5e7eb] px-4 py-2 text-[13px] font-medium text-[#374151] hover:bg-[#f9fafb] dark:border-neutral-600 dark:hover:bg-neutral-800">
          返回对话
        </Link>
        {session.canManageMembers ? (<Link href="/admin/users" className="rounded-full bg-[#4f46e5] px-4 py-2 text-[13px] font-medium text-white hover:bg-[#4338ca]">
            审核成员
          </Link>) : null}
        <LogoutLink />
      </nav>
