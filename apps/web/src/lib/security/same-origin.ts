...
const trustedHostRaw = (headers: Headers): string | null => {
    if (process.env.TRUST_PROXY_HEADERS === "true") {
        const xfh = headers.get("x-forwarded-host");
        if (xfh)
            return xfh.split(",")[0]?.trim().toLowerCase() ?? null;
    }
    return headers.get("host")?.trim().toLowerCase() ?? null;
};
const requestHostComparable = (hostRaw: string | null): string | null => {
    if (!hostRaw)
        return null;
    return hostRaw.toLowerCase();
};
const originsHostComparable = (urlStr: string): string | null => {
    try {
...
export const forbiddenIfUntrustedMutation = (req: Request): NextResponse | null => {
