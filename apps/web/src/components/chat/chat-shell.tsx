...
const sortConversationsForSidebar = (items: ConversationListItem[]): ConversationListItem[] => {
...
const AssistantPendingRow = () => {
    return (<li className="flex justify-start" aria-live="polite">
      <div className="flex items-center gap-2 rounded-2xl border border-[#e5e7eb] bg-white px-4 py-2.5 text-[14px] text-[#6b7280] shadow-sm">
        <span className="inline-block h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-[#e5e7eb] border-t-[#4f46e5]" aria-hidden/>
        <span>正在生成回复</span>
        <span className="inline-flex items-center gap-0.5" aria-hidden>
          {[0, 120, 240].map((delayMs) => (<span key={delayMs} className="inline-block h-1 w-1 animate-pulse rounded-full bg-[#9ca3af]" style={{ animationDelay: `${delayMs}ms` }}/>))}
        </span>
      </div>
    </li>);
};
...
const IconChat = ({ className }: {
    ...
    return (<svg className={className} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5Z"/>
    </svg>);
};
const IconSidebarToggle = ({ className }: {
    className?: string;
}) => {
    return (<svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
...
const IconPlus = ({ className }: {
    ...
    return (<svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
const IconMic = ({ className }: {
    ...
    return (<svg className={className} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
const IconPin = ({ active, className }: {
    active?: boolean;
    className?: string;
}) => {
    return (<svg className={className} width={17} height={17} viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.75" strokeLinejoin="round" aria-hidden>
      <path d="M12 2.7 14.02 9.06l7.06.61-5.41 4.62 1.71 6.93L12 18.56l-6.39 4.67 1.71-6.93-5.41-4.61 7.06-.61L12 2.7z"/>
    </svg>);
};
const IconEditTitle = ({ className }: {
    className?: string;
}) => {
    return (<svg className={className} width={17} height={17} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
      <path strokeLinecap="round" strokeLinejoin="round" d="m15.2 6.9 3.9-4L21 3l-.9 2-3.9 4M13 10l8-9-5-5-8 9v5h5Z"/>
    </svg>);
};
const fetchJson = async <T,>(url: string): Promise<{
    ok: true;
    data: T;
} | {
    ok: false;
...
const consumeSse = async (stream: ReadableStream<Uint8Array>, handle: (event: string, payload: unknown) => void): Promise<void> => {
...
const mutateJson = async <B, R,>(url: string, method: "POST" | "PATCH", body: B): Promise<{
    ok: true;
    data: R;
} | {
    ok: false;
...
export const ChatShell = ({ initialConversations, viewer }: ChatShellProps) => {
    ...
    const patchConversation = useCallback(async (id: string, body: {
        title?: string;
        pinned?: boolean;
    }): Promise<boolean> => {
    ...
    const togglePinOptimistic = useCallback(async (id: string) => {
        ...
        flushSync(() => {
            setConversations((prev) => {
                ...
                nextPinned = !t.pinned;
                return sortConversationsForSidebar(prev.map((c) => (c.id === id ? { ...c, pinned: nextPinned } : c)));
            });
        });
        if (!found)
            return;
        const result = await mutateJson<{
            pinned: boolean;
        }, PatchConversationOk>(`/api/conversations/${id}`, "PATCH", { pinned: nextPinned });
        if (!result.ok) {
            flushSync(() => {
        ...
        setConversations((cur) => sortConversationsForSidebar(cur.map((c) => c.id === id
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            setMessagesError(null);
            const result = await fetchJson<ChatMessage[]>(`/api/conversations/${activeConversationId}/messages`);
            if (cancelled)
                return;
            setMessagesLoading(false);
            if (result.ok) {
    ...
    const sendMessage = useCallback(async () => {
        ...
        try {
            ...
            if (!convId) {
                const created = await mutateJson<Record<string, unknown>, {
                    id: string;
    ...
    const isFreshNewChatUi = activeConversationId == null && !messagesLoading && messages.length === 0;
    /** 新开对话且尚未选定会话时的欢迎区 */
    ...
    return (<div className="flex h-dvh bg-[#f3f4f6] text-[#1f2937]">
      <aside className={[
        ...
        ].join(" ")} aria-hidden={sidebarCollapsed}>
        ...
        <ul className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto px-2 pb-3">
          {listLoading ? (<li className="px-3 py-6 text-center text-[13px] text-[#9ca3af]">加载列表中…</li>) : listError ? (<li className="px-3 py-4 text-[13px] text-red-600">
              <span className="block">{listError}</span>
              <button type="button" onClick={() => void loadConversations()} className="mt-2 text-[13px] font-medium text-[#4f46e5] hover:underline">
                重试
              </button>
            </li>) : conversations.length === 0 ? (<li className="px-3 py-6 text-center text-[13px] text-[#9ca3af]">暂无历史对话</li>) : (conversations.map((c) => {
            const selected = activeConversationId === c.id;
            const editing = editingSidebarId === c.id;
            return (<li key={c.id} className="group relative">
                  {editing ? (<form className="flex flex-col gap-2 rounded-xl border border-[#e5e7eb] bg-white px-2.5 py-2 shadow-sm" onSubmit={(e) => {
                        e.preventDefault();
                      ...
                      <input value={editSidebarTitleDraft} onChange={(e) => setEditSidebarTitleDraft(e.target.value)} className="w-full rounded-lg border border-[#e5e7eb] px-2 py-1.5 text-[13px] text-[#111827] outline-none focus:border-[#4f46e5]" autoFocus maxLength={512}/>
                      <div className="flex justify-end gap-2">
                        <button type="button" className="rounded-lg px-2 py-1 text-[12px] text-[#6b7280] hover:bg-[#f3f4f6]" onClick={() => setEditingSidebarId(null)}>
                          取消
                        </button>
                        <button type="submit" className="rounded-lg bg-[#4f46e5] px-2.5 py-1 text-[12px] font-medium text-white hover:bg-[#4338ca]">
                          保存
                        </button>
                      </div>
                    </form>) : (<div className={[
                      ...
                      <button type="button" onClick={() => {
                        ...
                        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-[#e8e8ea] bg-white text-[#a1a1aa]">
                          {c.pinned ? (<IconPin active className="h-3.5 w-3.5 text-amber-500"/>) : (<IconChat />)}
                        </span>
                      ...
                      <div className="flex shrink-0 items-center gap-0.5 pr-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100">
                        <button type="button" aria-label={c.pinned ? "取消置顶" : "置顶"} title={c.pinned ? "取消置顶" : "置顶"} onClick={(ev) => {
                        ev.preventDefault();
                        ev.stopPropagation();
                        ...
                        <button type="button" aria-label="修改标题" title="修改标题" onClick={(ev) => {
                        ev.preventDefault();
                        ev.stopPropagation();
        ...
        <div className="mt-auto border-t border-[#eceeef] p-3">
          <Link href="/me" className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-[13px] text-[#6b7280] transition-colors hover:bg-black/[0.04] hover:text-[#374151]">
          ...
          {viewer?.canManageMembers ? (<Link href="/admin/users" className="mt-2 block rounded-lg px-2 py-1.5 text-center text-[12px] font-medium text-[#4f46e5] hover:bg-[#eef2ff]">
              审核成员
            </Link>) : null}
        </div>
      </aside>
      ...
      <main className="relative flex min-w-0 flex-1 flex-col bg-white shadow-[inset_1px_0_0_rgba(0,0,0,0.04)]">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-[#f0f0f0] px-4">
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setSidebarCollapsed((v) => !v)} className="rounded-lg p-2 text-[#6b7280] hover:bg-black/[0.04] hover:text-[#374151]" aria-label={sidebarCollapsed ? "展开侧栏" : "收起侧栏"}>
              <IconSidebarToggle />
            </button>
            <button type="button" onClick={startNewChat} className="rounded-lg p-2 text-[#6b7280] hover:bg-black/[0.04]" aria-label="新对话">
              <IconPlus />
            </button>
            <div className="ml-1 flex min-w-0 flex-1 flex-col justify-center leading-tight">
              <span className="flex min-w-0 items-center gap-1">
                <span className="truncate text-[15px] font-semibold text-[#111827]">
                  {activeConversation?.pinned ? (<span className="mr-1 inline-block align-middle text-amber-500" title="已置顶">
                      <IconPin active className="inline align-[-3px]"/>
                    </span>) : null}
                  {activeConversation?.title ?? "新对话"}
                </span>
                {activeConversationId ? (<button type="button" aria-label="修改标题" className="shrink-0 rounded-md p-1 text-[#9ca3af] hover:bg-black/[0.06] hover:text-[#4f46e5]" onClick={() => {
                if (!activeConversationId)
                    return;
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {sendingFirstOnNewChat ? (<div className="flex flex-1 items-center justify-center px-6 pb-[18vh] text-[14px] text-[#9ca3af]">
              正在写入会话并调用模型…
            </div>) : showingEmptyLanding ? (<div className="flex flex-1 flex-col items-center justify-center px-6 pb-[18vh]">
              ...
              <div className="mt-10 grid w-full max-w-2xl grid-cols-1 gap-2 sm:grid-cols-2">
                {SUGGESTIONS.map((s) => (<button key={s} type="button" onClick={() => applySuggestion(s)} className="rounded-2xl border border-[#e5e7eb] bg-[#fafafa] px-4 py-3 text-left text-[14px] leading-snug text-[#374151] transition-colors hover:border-[#d1d5db] hover:bg-white">
                    {s}
                  </button>))}
              </div>
            </div>) : messagesLoading ? (<div className="flex flex-1 items-center justify-center text-[14px] text-[#9ca3af]">加载消息中…</div>) : messagesError ? (<div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center text-[14px] text-red-600">
              <span>{messagesError}</span>
              <button type="button" onClick={() => setMessagesRetryTick((n) => n + 1)} className="text-[13px] font-medium text-[#4f46e5] hover:underline">
                重试
              </button>
            </div>) : messages.length === 0 ? (<div className="flex flex-1 flex-col items-center justify-center px-6 pb-[18vh]">
              <h1 className="text-center text-[26px] font-semibold tracking-tight text-[#111827] sm:text-[30px]">
                有什么我能帮你的吗？
              </h1>
              <p className="mt-2 text-[14px] text-[#9ca3af]">该会话暂无消息</p>
            </div>) : (<div ref={messagesScrollRef} className="flex-1 overflow-y-auto px-4 py-6 sm:px-8">
              <ul className="mx-auto flex max-w-3xl flex-col gap-4">
                {messages.map((m) => (<li key={m.id} className={["flex", m.role === "user" ? "justify-end" : "justify-start"].join(" ")}>
                    <div className={[
                ...
                {thinkingPanelVisible && streamThinking.trim() ? (<li className="flex justify-start">
                ...
                {sendBusy && streamAnswerPreview ? (<li className="flex justify-start">
          ...
          <div className="shrink-0 border-t border-[#f3f4f6] bg-white px-4 pb-6 pt-4 sm:px-8">
            <div className="mx-auto max-w-3xl rounded-[22px] border border-[#e8e8e8] bg-[#fafafa] shadow-sm">
              {sendError ? (<div className="border-b border-red-100 px-4 py-2 text-[13px] text-red-600">{sendError}</div>) : null}
              <textarea value={draft} onChange={(e) => {
            setDraft(e.target.value);
            if (sendError)
                setSendError(null);
        }} disabled={sendBusy} placeholder={sendBusy ? "生成回复中…" : "发消息或输入 '/' 选择技能（Enter 发送，Shift+Enter 换行）"} rows={3} className="block w-full resize-none bg-transparent px-4 pb-2 pt-3 text-[15px] text-[#111827] outline-none placeholder:text-[#a1a1aa] disabled:opacity-50" onKeyDown={(e) => {
            if (e.key === "Enter" && !e.shiftKey) {
              ...
              <div className="flex items-center gap-2 border-t border-black/[0.04] px-3 py-2">
                <button type="button" className="rounded-lg p-2 text-[#9ca3af] hover:bg-black/[0.04] hover:text-[#374151]" aria-label="添加">
                  <IconPlus className="h-5 w-5"/>
                </button>
                <div className="flex flex-1 flex-wrap items-center gap-1 text-[13px] text-[#9ca3af]">
                  {["快捷", "编程", "图像", "写作"].map((t) => (<button key={t} type="button" className="rounded-full px-2.5 py-1 hover:bg-black/[0.04] hover:text-[#4b5563]">
                      {t}
                    </button>))}
                  <span className="px-1">…</span>
                </div>
                <button type="button" onClick={() => void sendMessage()} disabled={!draft.trim() || sendBusy} className="rounded-full bg-[#4f46e5] px-4 py-1.5 text-[13px] font-medium text-white disabled:opacity-40">
                  {sendBusy ? "发送中…" : "发送"}
                </button>
                <button type="button" className="rounded-lg p-2 text-[#9ca3af] hover:bg-black/[0.04]" aria-label="语音">
                  <IconMic />
                </button>
              </div>
            </div>
