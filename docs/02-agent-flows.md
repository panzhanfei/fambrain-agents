...
| 线 | 目录 | 编排 |
|----|------|------|
| **在线** | `agentflow/agents/online/` + `pipeline/graph/` | LangGraph：`prepareTurnStart` → … → `persistTurnEnd` |
...
实现：`apps/agents/src/agentflow/pipeline/graph/compile.ts` · SSE 壳 `pipeline/graph/stream.ts` → `runPipelineStream()`（SSE + 步骤耗时；业务在图内）。

**`stream.ts` 仅负责：** SSE 推送、`PipelineTimingTracker`、Pipeline 出去日志；**不**含 Mem0 读写业务。
