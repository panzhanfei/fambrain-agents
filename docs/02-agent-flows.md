...
| 英文名 | 中文名 | 职责 |
|--------|--------|------|
| **`TurnPrepare`** | **轮次准备** | LangGraph **首节点**（非 LLM）：挂 ALS 记事本、同问短路、Mem0/LangMem 注入 |
...
**里程碑：** 用户提问 → **轮次准备** → 意图识别 → 检索 → **证据核查** → **内容整理** → 分析 → 回答。（LangGraph 编排 **已实现**；KM：**向量 + 关键词 fallback**；FactChecker / ContentOrganizer：**D5/D6 已接入**；**Mem0/LangMem** 在 **PrepareTurn** 注入、供 Intake/Analyst 使用；跨轮 **两层 cache**（**同问短路** + **检索结果 cache**）见 [坑点 §2.2](./04-pitfalls.md)。）

**架构双线（2026-06）：**

| 线 | 目录 | 编排 |
|----|------|------|
| **在线** | `agentflow/agents/online/` + `pipeline/graph/` | LangGraph：`prepareTurn` → `intake` → … |
...
flowchart TB
  ...
  subgraph online ["在线：用户聊天 POST .../messages（LangGraph）"]
    U[用户消息] --> PT[TurnPrepare<br/>prepare_turn]
...
实现：`apps/agents/src/agentflow/pipeline/graph/compile.ts` · SSE 壳 `pipeline/graph/stream.ts` → `runPipelineStream()`（消费 LangGraph stream，**不含** L1/Mem0 业务逻辑）。

**D5-2 / P0-15 三层 cache（2026-06 · env 可关）：**

| 层 | 位置 | Key / 条件 | 命中后 | 关闭 |
|----|------|------------|--------|------|
| **同问短路** | **`prepare-turn`**（`prepareTurn` 节点） | `normalize(userQuestion)` + history 中已有 assistant 答 | 只 emit **`prepare_turn`**，复用上轮答案（`repeatQuestionHit`）→ `respondEarly` | `REPEAT_QUESTION_CACHE_DISABLED=1` |
| **检索结果 cache** | `retrievalNode` | `{prefix}:retrieval:v1:{corpusUserId}:{queryType}:{normalize(searchQuery)}` | 跳过 KM；仍走 FC / Analyst（`retrievalCacheHit`） | `RETRIEVAL_CACHE_DISABLED=1` |
...
```mermaid
flowchart TD
  A[用户消息] --> PT[TurnPrepare<br/>ALS + 同问短路 + Mem0]
...
**触发：** 每轮 LangGraph **必经**首节点 `prepareTurn`（`START → prepareTurn`）。**非 LLM**。
...
**代码：** `agentflow/agents/online/prepare-turn/` · 图节点 `compile.ts` · SSE step 名 **`prepare_turn`**（UI：准备上下文）

**验证：** `pnpm run verify:repeat-question-smoke`（同问短路，无 Ollama）；全链路 `verify:fact-checker:pipeline`（首步应为 `prepare_turn`）。
...
**触发：** 每轮 LangGraph **`prepareTurn` 节点**内调用 `preparePipelineMemory()`（`agentflow/agents/online/prepare-turn/prepare-turn.ts`）。**不参与**离线入库链路。
...
|---------|------|
| `meta` | 用户消息已落库（含真实 `id`） |
| `step` | 编排进度：**`prepare_turn`** / `intake` / **`user_fact`** / `retrieval` / `fact_checker` / **`content_summarizer`** / **`content_organizer`** / `analyst`，`status` 为 `running` \| `done`；`done` 时可带 `durationMs` |
