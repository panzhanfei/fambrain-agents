...
| Agent | 只负责 | 禁止 |
|--------|--------|------|
| IntakeCoordinator | **语义终稿 / 任务规划**：`intent`、`searchQuery`、`retrievalPlan`、`coreference` 等 | 写长答案、编造履历、口语二次拆槽、决定「下一个 Agent 名字」 |
...
| **Intake 架构** | 规划 / 发明槽 / 纠偏缠在同一条 guard 叙事里（含曾有的 `filled_fallback`） | 复盘成本高；违反「意图归 LLM」与 no-scene-hardcoding | **档 B**：LLM 出完整 `retrievalPlan` / `searchQuery` / `coreference`；代码只 schema 合法化、facet 去重、编译 PathPlan、少量 elliptical 纠偏 |
...
用户问句
  ...
  │
  ├─【主路径·规划】Intake LLM → JSON 语义终稿（intent + plan + coreference…）
  │     ├─ 非 JSON → 格式修复 1×（仍是要 JSON，不是二次猜意图）
  │     └─ JSON 且指代未消解 + 有上轮 → 拼接「上轮；本轮」再规划 1×
  │
  └─【旁路·纠偏】runIntakePipeline：parse 兜底 → continuation / chitchat / link /
        retrievalPlan 合法化 → composite → PathPlan 编译
```

复盘时：**先看 LLM 工单对不对 → 再看日志里哪一层纠偏改过字段**（`JSON格式修复重试` / `指代拼接重试` / `guard_*`），不要默认「代码又 Intake 了一次」。

#### 关键文件

| 文件 | 职责 |
|------|------|
| `contract/prompt.ts` | 档 B 契约、`coreference`、`COREFERENCE_MERGE_RETRY_NOTE` / `JSON_FORMAT_REPAIR_NOTE` |
| `signals/effective-intake-question.ts` | normalize、单字短路、`shouldRetryCoreferenceMerge`（只认 JSON peek） |
| `nodes/intake-node.ts` | 短路 → LLM → 格式修复 → 指代拼接 → pipeline |
| `pipeline/intake-pipeline.ts` | guard 链（纠偏 / 编译，不发明多槽） |
| `guards/intake-continuation-guard.ts` | 拼接后仍省略时的 elliptical 兜底（非主规划） |

**验证：** `pnpm exec vitest run apps/brain-service/tests/intake-coordinator/effective-intake-question.test.ts` · `pnpm --filter @fambrain/brain-service run verify:intake-coreference`。详见 [架构 v2 §13](./05-architecture-v2-tool-orchestration.md#13-intake-档-b主路径规划--旁路纠偏-2026-07)、[Intake README](../apps/brain-service/src/agentflow/agents/online/intake-coordinator/README.md)。
...
| 层 | 模块 | 职责 |
|----|------|------|
| **L0 入口 guard 链** | `intake-pipeline.ts` 等 | LLM 指代/clarify 早退 → chitchat → **retrievalPlan 合法化/canonicalize**（不发明槽）→ composite 路由 |
