...
| **Intake 架构** | 规划 / 发明槽 / 纠偏缠在同一条 guard 叙事里（含曾有的 `filled_fallback`） | 复盘成本高；违反「意图归 LLM」与 no-scene-hardcoding | **档 B**：`retrieve_and_answer` 须 LLM **`retrievalPlan≥1`**；代码只 schema 合法化、facet 去重、编译 PathPlan；空 plan → **clarify** |
