...
```bash
pnpm --filter @fambrain/agents run verify:r6-no-cache   # R6-1：同句再问 4 家一致（三层 cache 全关）
```
...
| 机制 | 模块 | 职责 |
|------|------|------|
| **同问短路** | `prepare-turn/repeat-question-guard.ts` | 同会话**字面相同**问 → 复用 history 整答；`REPEAT_QUESTION_CACHE_DISABLED=1` 关闭 |
...
pnpm --filter @fambrain/agents run verify:composite-incremental # composite 终稿 cache
pnpm --filter @fambrain/agents run verify:r6-no-cache           # P0-15 + R6-1/2/3 全链路（三层 cache 全关，11 项）
...
| **G5b** | 上文城管平台，「那个项目呢？」答 E-HR 等无关项目 | Intake searchQuery 未补全上文实体 | `intake-coreference-guard.ts` **`enrichSearchQueryFromHistory`**；Golden 断言须含城管/React 等；eval **`answerMustNotRe`** 在 **`answerRe` 已匹配**时不误杀含「指的是」的合理解析答（`assert-golden.ts`） |
...
| **可观测** | 本轮 `latencyMs≈42600`，非重复问快路径 | 同问短路命中时仅 **`prepare_turn`** step、`repeatQuestionHit=true`、~ms 级 |
...
```bash
pnpm --filter @fambrain/agents run eval:run -- --profile-only   # G-履历综合 t1～t4（cache 开，t2 期望同问短路）
...
**新一轮用户消息（即使用户字面上重复上一问）：** 默认整图重跑；**同问短路**（LangGraph **`prepareTurn` 节点** / `repeat-question-guard.ts`）若 `normalize(userQuestion)` 与本会话 history 中某轮 user 相同且其后有 assistant 答 → **短路**，只 emit **`prepare_turn`** step 并流式复用上轮答案（`repeatQuestionHit`）。否则若 Intake 产出相同 `searchQuery` + `queryType` → **检索结果 cache** 跳过 KM 向量检索，仍走 FC / Analyst。
...
| **+2** | **同问短路**：`normalize(userQuestion)` 与本会话 history 相同 → 复用上轮 assistant 答；`REPEAT_QUESTION_CACHE_DISABLED=1` 可关 | 跨轮 verbatim 重复 | `prepare-turn/repeat-question-guard.ts` + `prepareTurn` 节点 ✅ |
...
**验证脚本：** `pnpm run verify:fact-checker`、`pnpm run verify:analyst-empty-hits`、`pnpm run verify:intake-chitchat`、`pnpm run verify:repeat-question-smoke`、`pnpm run verify:retrieval-cache`、`pnpm run golden:regression`（`apps/agents/package.json`）。
...
| D3-9 | Analyst | 追问「那个项目呢」易 clarify 或答偏 | Analyst **不读**全量 DB 历史，仅 `userQuestion` + hits + **memoryBlock** | **coreference guard** `enrichSearchQueryFromHistory`；Golden **G5b** 断言收紧 | ✅ **已缓解**（2026-06 · Golden G5b） |
...
- [x] D5-2：同会话连续两问 G4 原文，第二次检索 cache 或同问短路 ← §2.2（2026-06-18）
