...
| 槽答案缓存命中 + `routeMode=slot` →「未标注年龄」 | 槽答案缓存跳过 KM，`merge.hits` 空 → `rules_empty_hits_skip_llm` | `knowledge-manager/composite/retrieve.ts`：citations→hits；`runtime/stream.ts` 单槽槽答案直出 | ✅ |
